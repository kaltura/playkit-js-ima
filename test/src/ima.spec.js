...
describe('Ima Plugin', function() {
  ...
  it('playback should keeps playing after midroll finished with playAdsWithMSE', done => {
    player = loadPlayerWithAds(
      ...
      {playback: {playAdsWithMSE: true}}
    );
    ima = player._pluginManager.get('ima');
    const keepsPlayingAfterAd = () => {
      player.removeEventListener(player.Event.AD_BREAK_END, keepsPlayingAfterAd);
      player.currentTime = 15;
      player.addEventListener(player.Event.AD_BREAK_END, () => {
        player.addEventListener(player.Event.PLAYING, () => done());
      });
    };
    player.addEventListener(player.Event.AD_BREAK_END, keepsPlayingAfterAd);
    player.play();
  });

  it('playback should keeps playing after postroll finished with playAdsWithMSE', done => {
    player = loadPlayerWithAds(
      targetId,
      {
        adTagUrl:
    ...
    ima = player._pluginManager.get('ima');
    const playing = () => {
      player.removeEventListener(player.Event.PLAYING, playing);
    ...
    player.addEventListener(player.Event.PLAYING, playing);
    player.play();
  });

  it('playback should not be affected with not linear ad and playAdsWithMSE', done => {
    player = loadPlayerWithAds(
      targetId,
      {
        adTagUrl:
